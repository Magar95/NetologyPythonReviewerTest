class Student:
    ...
    def __str__(self):
        return (f'Имя: {self.name} \n'
                ...
                f'Курсы в прогрессе изучения: {", ".join(self.courses_in_progress)}\n' # необходим метод join для превращения списка в строку
                f'Завершенные курсы: {", ".join(self.finished_courses)}\n') # необходим метод join для превращения списка в строку

    def __eq__(self, other):
        return self.average_rate_student() == other.average_rate_student() # во втором операнде вместо self должно быть other
...
class Lecturer(Mentor):
    ...
    def __eq__(self, other):
        return self.average_rate_lecturer() == other.average_rate_lecturer() # во втором операнде вместо self должно быть other
    ...
    def average_rate_lecturer(self):
        ...
        if len(rate_list) >= 2:
            ...
            raise Exception('Недостаточно оценок для расчета среднего балла')  # Не было вызвано исключение Exception
